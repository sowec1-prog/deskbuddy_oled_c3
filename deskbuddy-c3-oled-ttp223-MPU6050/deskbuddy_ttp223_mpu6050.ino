...
bool mpuReady = false;
float previousAccelerationG = 1.0f;
uint32_t lastMpuSampleAt = 0;
...
bool initMpu6050() {
  if (!MPU6050_ENABLED) return false;
  Wire.beginTransmission(MPU6050_ADDRESS);
  if (Wire.endTransmission() != 0) {
    Serial.println("MPU6050 nenalezen na I2C 0x68");
    return false;
  }
  // Wake device and use its default +-2 g accelerometer range.
  if (!writeMpuRegister(0x6B, 0x00) || !writeMpuRegister(0x1C, 0x00)) {
    Serial.println("MPU6050 nelze nastavit");
    return false;
  }
  Serial.println("MPU6050 pripraven: zatreseni aktivni");
  return true;
}
...
bool readAccelerationG(float &x, float &y, float &z) {
  ...
  if (Wire.endTransmission(false) != 0 || Wire.requestFrom(MPU6050_ADDRESS, (uint8_t)6) != 6) return false;
...
void handleMpu6050() {
  ...
  if (!mpuReady || now - lastMpuSampleAt < MPU6050_SAMPLE_INTERVAL_MS) return;
  lastMpuSampleAt = now;
  float x, y, z;
  if (!readAccelerationG(x, y, z)) return;
  const float accelerationG = sqrtf(x * x + y * y + z * z);
  const float deltaG = fabsf(accelerationG - previousAccelerationG);
  previousAccelerationG = previousAccelerationG * 0.75f + accelerationG * 0.25f;
  if (deltaG < MPU6050_SHAKE_DELTA_G || now - lastShakeAt < 900) return;
  ...
  leftEye.targetY = rightEye.targetY = -3.0f;
  Serial.printf("MPU6050: zatreseni %.2f g\n", deltaG);
}
